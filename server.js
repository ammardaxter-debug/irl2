...
function getSprintState(nowStr) {
  return {
    phase: 'DISABLED',
    countdownMs: 0,
    currentSprintStart: new Date(),
    currentSprintEnd: new Date(),
    sprintNumber: 0
  };
}

app.get('/api/cron/sprint-manager', async (req, res) => {
  console.log('[CRON] Smart sprint manager requested (sprints are disabled).');
  return res.json({ success: true, message: 'Weekly Sprint is disabled.' });
});
