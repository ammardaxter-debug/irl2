    ...
    return `
      ${isMechanic ? '' : `
        <div class="fleet-tabs-nav" style="display:flex; gap:16px; border-bottom:1px solid var(--border-light); padding-bottom:0px; margin-bottom:20px;">
          <button class="fleet-tab-btn ${this.currentSubTab === 'overview' ? 'active' : ''}" data-subtab="overview" style="background:none; border:none; padding:12px 16px; font-weight:600; font-size:14px; color:${this.currentSubTab === 'overview' ? 'var(--primary-500, #2563eb)' : 'var(--text-secondary)'}; border-bottom:${this.currentSubTab === 'overview' ? '3px solid var(--primary-500, #2563eb)' : 'none'}; cursor:pointer; transition: all 0.2s;">Fleet Overview</button>
          <button class="fleet-tab-btn ${this.currentSubTab === 'maintenance' ? 'active' : ''}" data-subtab="maintenance" style="background:none; border:none; padding:12px 16px; font-weight:600; font-size:14px; color:${this.currentSubTab === 'maintenance' ? 'var(--primary-500, #2563eb)' : 'var(--text-secondary)'}; border-bottom:${this.currentSubTab === 'maintenance' ? '3px solid var(--primary-500, #2563eb)' : 'none'}; cursor:pointer; transition: all 0.2s; display:flex; align-items:center; gap:6px;">
      ...
      toolbarCont.innerHTML = `
        <div class="fleet-toolbar" style="display:flex; flex-direction:column; gap:12px; width: 100%; padding: 4px 0;">
          ...
          <div style="display:flex; gap:8px; overflow-x:auto; width: 100%; padding-bottom: 8px; scrollbar-width: none; -ms-overflow-style: none;">
            ...
            <div class="scroll-hide" style="display:flex; gap:8px; overflow-x:auto; padding-bottom: 2px; width: 100%; flex-wrap: nowrap;">
              <button class="btn btn-sm ${this.currentFilter === 'all' ? 'btn-primary' : 'btn-outline'}" onclick="Bikes.filterMaintenance('all')" style="white-space: nowrap; border-radius: 20px; padding: 6px 14px; font-size: 13px; flex-shrink: 0;">All (${this.maintenanceRequests.length})</button>
              <button class="btn btn-sm ${this.currentFilter === 'pending' ? 'btn-primary' : 'btn-outline'}" onclick="Bikes.filterMaintenance('pending')" style="white-space: nowrap; border-radius: 20px; padding: 6px 14px; font-size: 13px; flex-shrink: 0;">Pending (${this.maintenanceRequests.filter(r => r.status === 'pending').length})</button>
              <button class="btn btn-sm ${this.currentFilter === 'in-progress' ? 'btn-primary' : 'btn-outline'}" onclick="Bikes.filterMaintenance('in-progress')" style="white-space: nowrap; border-radius: 20px; padding: 6px 14px; font-size: 13px; flex-shrink: 0;">In Progress (${this.maintenanceRequests.filter(r => r.status === 'in-progress').length})</button>
              <button class="btn btn-sm ${this.currentFilter === 'waiting-for-parts' ? 'btn-primary' : 'btn-outline'}" onclick="Bikes.filterMaintenance('waiting-for-parts')" style="white-space: nowrap; border-radius: 20px; padding: 6px 14px; font-size: 13px; flex-shrink: 0;">Waiting for Parts (${this.maintenanceRequests.filter(r => r.status === 'waiting-for-parts').length})</button>
              <button class="btn btn-sm ${this.currentFilter === 'resolved' ? 'btn-primary' : 'btn-outline'}" onclick="Bikes.filterMaintenance('resolved')" style="white-space: nowrap; border-radius: 20px; padding: 6px 14px; font-size: 13px; flex-shrink: 0;">Resolved (${this.maintenanceRequests.filter(r => r.status === 'resolved').length})</button>
            </div>
          </div>
        </div>
      `;
