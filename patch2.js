...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('// 📝 SHEET 3: Summary 📝') || lines[i].includes('// ── SHEET 3: Summary ──')) {
        startIdx = i;
    }
    if (startIdx !== -1 && i > startIdx && lines[i].includes('ws3.views = [{state:\'frozen\',ySplit:4}];')) {
        endIdx = i;
        break;
    }
}
...
console.log('Successfully patched expenses.js line-by-line');
